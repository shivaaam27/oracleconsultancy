# Capture Setup — getting things into Oracle

The Oracle Inbox receives forwarded emails and shared messages through one secure
...
## WhatsApp / Share → Oracle (iPhone, via Apple Shortcut)
...
2. Tap the name at the top → rename to **Save to Oracle**.
...
6. *(Optional but nice)* Add a final action **Show Notification** with text
   `Saved to Oracle` so you get instant confirmation.
7. Tap **Done**.

### Use it

1. In WhatsApp, **select one or more messages** → tap **Share / Forward → Share**.
2. In the share sheet, scroll to **Save to Oracle** and tap it.
3. Oracle receives it and sends you a push: **"New item to file."**
4. Tap the push (or open Oracle → **Inbox**) → **File it** → it's pre-filled in the
   Capture Wizard, ready to become a task or note.

### What works today
- **Text / messages:** fully supported. Multiple selected messages arrive as one
  block of text — Oracle reads them all and strips WhatsApp's `[time, date] Name:`
