...
export function CocozuriBooksStrip({
  ...
  return (
    <div className={cn("flex flex-wrap items-center gap-x-3 gap-y-1.5 rounded-lg border px-3 py-2 text-sm print:hidden", tone)}>
      ...
      {state === "posted" && (
        <>
          <Link href="/ledger/entries" className="text-sm underline-offset-2 hover:underline">
            See the entries
          </Link>
          <button type="button" onClick={() => void reverse()} disabled={busy}
            className="inline-flex h-8 items-center gap-1.5 rounded-md border border-current/30 px-2 text-sm font-medium hover:bg-current/10 disabled:opacity-60">
      ...
      {state === "unposted" && !ready && (
        <Link href="/ledger?co=" className="inline-flex h-8 items-center rounded-md border border-border px-2 text-sm font-medium text-fg-muted hover:text-fg">
          Set the ledger up
        </Link>
      )}
    </div>
