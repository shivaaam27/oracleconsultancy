...
import type { CompanyGovernance } from "@/lib/companies/governance-shared";

const fmtDate = (iso: string | null) =>
  iso ? new Date(iso).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" }) : "";
const num = (n: number | null) => (n == null ? "—" : n.toLocaleString("en-GB"));
const input = "rounded-md border border-border bg-bg px-2 py-1 text-sm outline-none focus:border-accent/60";
  ...
  return (
    <section className="glass elevated rounded-2xl p-4 space-y-4">
      <h2 className="inline-flex items-center gap-1.5 text-sm font-semibold">
        ...
        <span className="ml-2 text-xs font-normal uppercase tracking-wider text-fg-subtle inline-flex items-center gap-1">
...
function AddToggle({ on, onClick, className }: { on: boolean; onClick: () => void; className?: string }) {
  return <button type="button" onClick={onClick} className={`inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-xs text-fg-subtle hover:bg-bg-muted ${className ?? ""}`}>{on ? <X size={11} /> : <Plus size={11} />} {on ? "Close" : "Add"}</button>;
}
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5 rounded-lg bg-bg-subtle/40 p-2">
      ...
      <button type="button" onClick={() => start(async () => { await addCapHolderAction(companyId, holder, shares ? Number(shares) : null, pct ? Number(pct) : null, null); onDone(); onSaved(); })}
        className="rounded-md bg-accent px-2 py-1 text-xs text-accent-fg">Add</button>
    </div>
  );
}
function SigForm({ companyId, onDone, onSaved }: { companyId: number; onDone: () => void; onSaved: () => void }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5 rounded-lg bg-bg-subtle/40 p-2">
      ...
      <button type="button" onClick={() => start(async () => { await addSignatoryAction(companyId, name, scope); onDone(); onSaved(); })} className="rounded-md bg-accent px-2 py-1 text-xs text-accent-fg">Add</button>
    </div>
  );
}
function ResForm({ companyId, onDone, onSaved }: { companyId: number; onDone: () => void; onSaved: () => void }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-1.5 rounded-lg bg-bg-subtle/40 p-2">
      ...
      <button type="button" onClick={() => start(async () => { await addResolutionAction(companyId, date || null, type, summary); onDone(); onSaved(); })} className="rounded-md bg-accent px-2 py-1 text-xs text-accent-fg">Add</button>
    </div>
  );
}
