...
import { requestCreate } from "@/lib/hooks/use-create-param";
import { useRegisteredActions } from "@/components/kit/context-actions";
...
export function StudioQuickAdd({ onClose, initialTab }: { onClose: () => void; initialTab?: string }) {
  ...
  return (
    <div className="fixed inset-0 z-[45]" role="dialog" aria-label="Create something">
      ...
      <div className="studio st-sheet st-sheet-dots st-pop absolute bottom-[calc(var(--foot-h)+var(--foot-safe)+8px)] right-3 flex max-h-[calc(100dvh-100px)] w-[min(720px,calc(100vw-24px))] flex-col gap-4 overflow-y-auto rounded-3xl bg-[var(--sh-bg)] p-5 text-[var(--sh-fg)] shadow-[0_30px_80px_rgba(0,0,0,0.4)] sm:right-6">
