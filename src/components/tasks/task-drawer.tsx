...
import { canStepBack, clearPush, returnLabel, safeReturn } from "@/lib/nav/return-to";
import { cachedTaskDetail, prefetchTaskDetail, storeTaskDetail } from "@/lib/tasks/task-detail-cache";
import { buildSections } from "../kit/entity-cells";
...
import { TaskCopyToCompanies, type CopyActions } from "@/components/tasks/task-copy-companies";
import { setTaskRecurrence, stopTaskRecurrence } from "@/app/task/recurring-actions";
import { RecurringTaskSheet, draftFromRule, scheduleLabel, BLANK as BLANK_RULE } from "@/components/portal/portal-recurring-tasks";
import type { RecurringTaskRule } from "@/lib/tasks/recurring-task-rules";
...
import { LinkedNotesTab } from "../notes/linked-notes";
...
function TaskRecord({ mode, codeProp, stamp }: { mode: "drawer" | "page"; codeProp?: string; stamp?: number }) {
  ...
  // at once instead of saying "Loading…" (lib/tasks/task-detail-cache.ts).
  ...
  const close = useCallback(() => {
    if (mode === "page") {
       ...
       * lands on the right page. Same rule as `components/shell/back-link.tsx`. */
