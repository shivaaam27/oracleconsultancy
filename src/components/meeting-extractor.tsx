...
import { describeProseFlags } from "@/lib/ai-verify";
...
export function MeetingExtractor({ companies, meetings, voiceLanguage = "en-GB", openId }: Props) {
  ...
  function handleVoiceStop() {
    ...
    startVoicePolish(async () => {
      ...
      flash(result.source === "ai" ? `Dictation polished by ORI.${tidied}` : result.source === "no-key" ? "AI is off — applied basic clean-up." : result.source === "error" ? "AI clean-up failed — applied basic clean-up." : "Dictation cleaned.");
    });
  }
  ...
  function assistGenerateMinutes() {
    ...
    startAssist(async () => {
      ...
      setPane("minutes");
      flash((result.source === "ai" ? "Minutes generated by AI." : result.source === "no-key" ? "AI is off — used the basic minutes builder." : result.source === "error" ? "AI minutes failed — used the basic builder." : (result.message || "Minutes generated.")) + describeProseFlags(result.flags));
    });
  }
  ...
  function assistCleanNotes() {
    ...
    startAssist(async () => {
      ...
      setPane("notes");
      flash((result.source === "ai" ? "Notes cleaned by AI." : result.source === "no-key" ? "AI is off — used the basic cleaner." : result.source === "error" ? "AI clean-up failed — used the basic cleaner." : (result.message || "Notes cleaned.")) + describeProseFlags(result.flags));
    });
  }
  ...
  function assistInsight(kind: "decisions" | "risks" | "follow-up") {
    ...
    startAssist(async () => {
      ...
      flash((result.source === "ai" ? `${label} added by AI.` : result.source === "no-key" ? `AI is off — used the basic ${label.toLowerCase()} builder.` : result.source === "error" ? `AI ${label.toLowerCase()} failed — used the basic builder.` : (result.message || `${label} added.`)) + describeProseFlags(result.flags));
    });
  }
                ...
                <div className="flex-1 flex flex-col">
                  ...
                  {showTeach && (
                    <div className="flex gap-2 pt-2">
                      <input value={voiceTerm} onChange={e => setVoiceTerm(e.target.value)} placeholder="Name or phrase ORI should remember" className="min-w-0 flex-1 rounded-lg border border-border bg-bg px-2.5 py-1.5 text-xs focus:outline-none focus:border-accent" />
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.14 }}
            className="absolute right-0 top-full mt-1 z-30 w-52 rounded-xl glass shadow-lg overflow-hidden py-1">
