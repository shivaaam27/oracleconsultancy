...
export function ProjectSnapshotSheet({
  ...
  return (
    <div className="space-y-4">
      ...
      {over.length > 0 && (
        <p className="flex items-start gap-1.5 rounded-md bg-danger-soft px-2.5 py-1.5 text-xs text-danger">
      ...
      <section className="overflow-hidden rounded-lg border border-border bg-bg-elev">
        <header className="flex items-baseline justify-between border-b border-border bg-bg-subtle px-3 py-2">
          <h3 className="text-sm font-medium">Budget against actual, by category</h3>
          <span className="text-xs text-fg-subtle">worst first · {gauge.length} categories</span>
        </header>
        {gauge.length === 0 ? (
      ...
      <section className="overflow-hidden rounded-lg border border-border bg-bg-elev">
        <header className="flex items-baseline justify-between border-b border-border bg-bg-subtle px-3 py-2">
          <h3 className="text-sm font-medium">Payment plan</h3>
        ...
        {stages.length === 0 ? (
          <div className="px-3 py-6 text-center">
            <p className="text-sm text-fg-subtle">No payment plan yet.</p>
...
function GaugeLine({ row }: { row: GaugeRow }) {
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-1.5">
      <span className="w-40 shrink-0 truncate text-sm" title={row.category}>{row.category}</span>
      ...
      <span className="tabular w-24 shrink-0 text-right text-xs text-fg-muted">{money(row.actual)}</span>
      ...
      <span className="tabular hidden w-12 shrink-0 text-right text-xs text-fg-subtle sm:block"
        title="Share of the whole budget">
        {row.share > 0 ? pct(row.share, row.share < 0.01 ? 1 : 0) : "—"}
      </span>
      <span className={cn("tabular w-14 shrink-0 text-right text-xs font-medium", TONE_TEXT[tone])}>
  ...
  return (
    <div className="px-3 py-2">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className={cn("shrink-0 rounded-sm px-1.5 py-0.5 text-xs font-medium",
        ...
        <span className="tabular w-28 shrink-0 text-right text-sm">{money(view.amount) ?? "—"}</span>
        <span className="tabular w-28 shrink-0 text-right text-xs text-fg-muted">
        ...
        <button type="button" onClick={() => setOpen((v) => !v)}
          className="shrink-0 rounded border border-border px-1.5 py-0.5 text-xs text-fg-muted hover:text-fg">
          {open ? "Close" : "Invoice"}
        </button>
      </div>
...
function Mark({ on, label, title }: { on: boolean; label: string; title: string }) {
  return (
    <span title={`${title}: ${on ? "yes" : "no"}`}
      className={cn("rounded-sm px-1 py-0.5 text-xs font-medium",
...
function Toggle({ on, onChange, label }: { on: boolean; onChange: (v: boolean) => void; label: string }) {
  return (
    <button type="button" role="switch" aria-checked={on} onClick={() => onChange(!on)}
      className={cn("h-8 rounded-md border px-2 text-xs",
...
const smallInput =
  "h-8 w-32 rounded-md border border-border bg-bg px-2 text-sm outline-none focus:border-accent";

function Small({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <span className="mb-1 block text-xs uppercase tracking-[0.04em] text-fg-subtle">{label}</span>
      {children}
    </label>
  );
}
...
function Tile({ label, value, sub, tone, small }: {
  ...
  return (
    <div className="bg-bg-elev px-3 py-2">
      ...
      <p className={cn("tabular mt-0.5", small ? "text-base" : "text-[15px]",
