 ...
 */
export function ExtractionHealth({ className = "", bare = false }: { className?: string; bare?: boolean }) {
  ...
  if (!health) return bare ? <p className="text-xs text-fg-muted">Loading…</p> : null;
  if (health.total === 0) {
    if (bare) return <p className="text-xs text-fg-muted">No document reads yet.</p>;
    return (
  ...
  // The body content, shared between the standalone card and the bare (in-card) form.
  const body = (
    <>
      {byReason.length > 0 ? (
