  ...
  return (
    <section className="glass elevated rounded-2xl p-4 space-y-2.5">
      <h2 className="inline-flex items-center gap-1.5 text-xs font-medium uppercase tracking-wider text-fg-muted">
        ...
        <span className="text-xs font-normal text-fg-subtle normal-case">inferred from documents</span>
      </h2>
      <ul className="divide-y divide-border/50">
        {relationships.map((r, i) => (
          <li key={`${r.role}-${r.personId ?? r.name}-${i}`} className="flex items-center gap-2.5 py-2">
            <span className={`shrink-0 inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ring-1 ${ROLE_TONE[r.role] ?? "bg-bg-subtle text-fg-muted ring-border/60"}`}>
              {r.role}
            </span>
            <span className="min-w-0 flex-1">
              ...
              {r.detail && <span className="ml-1.5 text-xs text-fg-subtle">{r.detail}</span>}
            </span>
            {!r.personId && <span className="shrink-0 text-xs text-fg-subtle">not on file</span>}
          </li>
        ))}
      </ul>
    </section>
