...
//
// Both the per-write hooks (src/lib/search/index-hooks.ts) and the nightly catch-all
// (src/lib/search/embeddings-reindex.ts) derive from this one table, so they can never
...
import type { SourceType, Lifecycle } from "@/lib/search/embeddings";
...
//
// One SearchResult is what the deep-index search (src/lib/search/search.ts) and the
...
// canonical sets in src/lib/search/search.ts and src/lib/search/embeddings-reindex.ts so
 ...
 * mapping OR a `searchCustom` escape-hatch), in palette order. src/lib/search/search.ts
