...
export async function createGoogleEvent(
  ...
  try {
    ...
    const end = ev.endAt
      ? new Date(ev.endAt)
      : new Date(start.getTime() + (ev.allDay ? 24 : 1) * 60 * 60 * 1000);

    // All-day events use date-only; timed events use RFC3339 with the EAT zone.
    const startField = ev.allDay
      ? { date: start.toISOString().slice(0, 10) }
      : { dateTime: start.toISOString(), timeZone: EAT_TZ };
    const endField = ev.allDay
      ? { date: end.toISOString().slice(0, 10) }
      : { dateTime: end.toISOString(), timeZone: EAT_TZ };

    const wantMeet = opts?.requestMeet && !ev.meetLink;

    const requestBody: calendar_v3.Schema$Event = {
      summary: ev.title,
      description: [ev.description, ev.meetLink ? `Join: ${ev.meetLink}` : null]
        .filter(Boolean)
        .join("\n\n") || undefined,
