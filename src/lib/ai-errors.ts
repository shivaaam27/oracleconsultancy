...
export function friendlyAIError(raw: string | null | undefined): FriendlyError {
  ...
  if (groq) {
    ...
    if (status === 429 || status === 413)
      return { message: "AUMIO is busy right now — give it a moment and try again.", retryable: true };
    if (status === 401 || status === 403)
      return { message: "AUMIO's AI key isn't working. Check the AI key in Settings.", retryable: false };
    if (status >= 500)
      return { message: "The AI service had a hiccup. Try again in a moment.", retryable: true };
    return { message: "AUMIO couldn't complete that. Try rephrasing, or try again.", retryable: true };
  }
  ...
  switch (code) {
    ...
    case "bad-json":
      return { message: "AUMIO didn't quite understand that. Try rephrasing it.", retryable: true };
    case "network error":
    case "network":
