// Probation reminders — a daily HR summary to the owner. (Leave approvals were
// dropped Sept 2026: the leave module is retired, so there is never any to approve.)
...
export const lifecycleCategory: CategoryDef = {
  ...
  async run(ctx) {
    ...
    const r = await ctx.sendToOwner("HR reminders — probation", text, "automation-lifecycle", {
      doc: {
        preheader: `${bullets.length} HR item${bullets.length === 1 ? "" : "s"} need attention.`,
        title: "HR reminders",
        subtitle: `Probation · as at ${data.asAt}`,
        blocks: [{ kind: "list", bullets }],
        cta: { label: "Open the HR area", url: `${appBaseUrl()}/people` },
        footerNote: "You're receiving this because the probation reminders automation is on. Manage in Settings → Email automation.",
        office: "hr",
      },
    });
