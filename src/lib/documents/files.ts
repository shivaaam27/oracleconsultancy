...
import { deleteDocumentForever } from "@/lib/documents/documents";
import { extOf, KEEP_DELETED_DAYS, type FileRow, type FileStatus, type FolderColor, type FolderRow, type Library } from "@/lib/documents/files-shared";
...
export async function viewerLibrary(v: import("@/lib/auth/viewer").Viewer, libIn: Library | Promise<Library>): Promise<Library> {
  if (v.kind === "owner") return libIn;
  const { viewerPeopleIds } = await import("@/lib/auth/viewer-scope");
...
export async function viewerCanSeeDocument(v: import("@/lib/auth/viewer").Viewer, docId: number): Promise<boolean> {
