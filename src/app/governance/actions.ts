...
export async function deleteCapHolderAction(id: number, companyId: number) {
  await guardOwner(); await deleteCapHolder(id); revalidateGov(companyId); return { ok: true }; }
...
export async function deleteSignatoryAction(id: number, companyId: number) {
  await guardOwner(); await deleteSignatory(id); revalidateGov(companyId); return { ok: true }; }
...
export async function deleteResolutionAction(id: number, companyId: number) {
  await guardOwner(); await deleteResolution(id); revalidateGov(companyId); return { ok: true }; }
...
export async function decideDecisionAction(id: number, decision: string) {
  await guardOwner(); await decideDecision(id, decision.trim() || "Decided"); revalidateGov(); return { ok: true }; }
