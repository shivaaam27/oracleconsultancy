import { pgTable, serial, integer, text, boolean, timestamp, doublePrecision, bigint, numeric, jsonb, primaryKey, uniqueIndex, index, type AnyPgColumn } from "drizzle-orm/pg-core";
...
// AI job queue — was the bridge between the live app and the Claude Code cloud
// agent: the app enqueued a job, a cloud routine claimed it via
// claim_next_ai_job(), did the work and wrote back `result`. The cloud worker was
// removed (Sept 2026); the table is kept, unreachable.
...
// conversation (see memory/ori_automations.md). Each row is one rule
