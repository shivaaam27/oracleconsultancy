...
rules = [('Nothing is deleted until you say so', 'Every page is switched on beside the old one. You try it, then we switch. The old page is removed only after a week of the new one in use.'),
         ('The saving code does not change', 'New screens call the same save, update, post and delete actions COS uses today. Where a page needs to save one field at a time, it gets a small new wrapper over the same core — never a second way of writing.'),
