# Builds ONE self-contained page of the STAFF boards (S_*.dc.html) for
# publishing as a private link. python preview/build_staff.py <out.html>
import glob, json, os, sys
S = os.path.dirname(os.path.abspath(__file__))
P = os.path.join(S, '..', 'boards')
ORDER = ['S_Rules', 'S_Home', 'S_Tasks', 'S_Task', 'S_Profile']
LABEL = {'S_Rules': 'The plan', 'S_Home': 'Home', 'S_Tasks': 'Tasks', 'S_Task': 'A task', 'S_Profile': 'Profile'}
...
html = '''<title>Staff Studio Boards</title>
<style>
...
<div class="top"><div class="row"><div class="ttl">Staff on the new screens<span>desk &amp; phone · 5 boards</span></div>
...
<script>window.BOARDS=''' + json.dumps(boards).replace('</', '<\\/') + ''';window.START="S_Rules.dc.html";
var mode='fit';
