...
- **T2 Conversation timeline** — IN PROGRESS. Done: replies (1 level, `task_updates.parent_update_id`, migration 0041), pinned-instruction banner, chat-style composer with reply targeting, on the PORTAL task page via `src/components/portal-conversation.tsx` (client; server precomputes `ConvoMessage[]`). Reply quotes also render on admin `/task/[code]`. Still to do: @mentions, drag-drop attachments (→ Documents), system events as thin inline markers, voice in composer, and bringing the same composer to the admin page (overlaps T3).
