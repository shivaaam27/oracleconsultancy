...
- **More company-drawer entry points** — wire `CompanyDrawerLink` (`?company=ID`) into other places company names appear so the quick-view opens consistently: e.g. the company link inside the **task drawer** hero, task-table company cells, person-drawer "company" line, Org chart, Home/Brief. Currently the drawer only opens from the `/companies` hub cards; everywhere else still navigates to the full page.
