...
### Drawer redesign — `EntityDrawer` cockpit (owner: "the profile drawer is ugly")
Owner hated the person drawer (7 identical stacked grey accordions; edit was a full-drawer mode; pack buried; no 1-3-click hierarchy). Locked decisions: **segmented tabs** (one view at a time), **inline edit in place**, **build a reusable shell** (person first; company + pop-ups adopt next).
- **New reusable shell** `src/components/entity-drawer.tsx`: rounded-3xl glass dialog with a status-tinted hero glow, a glass **segmented tab pill** (same pill as company tabs), an **all-tabs-mounted** body (inactive `hidden` → summaries report + instant switching, active tab gets a fade/slide entrance), and a **sticky action bar**. Props: `title` (sr-only Dialog.Title for a11y), `tone`, `hero`, `tabs[]`, `activeTab`/`onTabChange`, `actionBar`, `loading`/`error`. Reuse this for the company drawer and pop-ups.
- **Person drawer rebuilt** on it (`person-drawer.tsx`): hero (initials avatar tinted by compliance band, name, role · company · ↳ manager, type chip, Email/WhatsApp/Call); tabs **Overview · Compliance · Journey(+equipment) · Tasks · Details**. Overview = compliance ring card + 4 stat tiles + a merged **"Needs attention"** list (overdue tasks + expired/expiring docs + missing-required → each jumps to the right tab). Editing is **inline in Details** (pencil → `PersonForm` in place, no full-drawer swap). Pack/Add-doc/Remind live in the sticky action bar. Old 7-accordion stack removed.
- Verified in preview: tabs switch instantly, compliance summary/ring/avatar tone populate (Risk → red), Needs-attention lists the 11 missing docs, Details shows profile + documents + manage + inline edit, no console errors. tsc clean.
- **Next:** apply `EntityDrawer` to the company view (drawer/page) and other pop-ups for one consistent feel.

### Drawer internals polish (owner: "sections still look old/traditional, too much negative space")
Locked decisions: header contact = **compact icon buttons**; **Prepare pack folded into the drawer as a step**; **one-tap Remind** with inline confirmation (drop the duplicate pack "Work reminder" reason + the redundant pack "Custom" reason — the "Customise sections" panel is the custom path). Phased build: 1 primitives kit + header + footer (DONE) · 2 Compliance + Journey (DONE) · 3 Details + Tasks (DONE) · 4 Pack rebuilt in-drawer (DONE) · 5 task drawer adopts the kit (DONE). **All 5 phases shipped.**

**Phase 1 shipped (foundation):** new `src/components/drawer-kit.tsx` — shared primitives `IconButton`, `ProgressTrack`, `SectionPulse`, `StatusChip`, `DrawerRow` (hover-revealed actions), `EmptyState`, `SectionCard`. Person drawer header now uses compact circular contact icons; footer redesigned (primary Prepare pack + right-aligned icon cluster); **Remind is one-tap** → drafts and shows an inline success strip (Outbox / Send / dismiss) instead of auto-opening; "Needs attention" all-clear uses the celebratory `EmptyState`. tsc clean; verified in preview.

**Phase 2 shipped (Compliance + Journey):** both rebuilt tab-native on the kit (no more outer `<details>`). **Compliance** (`requirements-checklist.tsx`): a progress-pulse header (score% + ProgressTrack + "X/Y verified · N missing" + Sync), then **Needs action** (missing-first) with an inline contextual primary button (Add / Verify / Renew), status badge, and tap-to-expand secondary actions (link/requested/unlink/waive/edit/remove); **In place** group collapsed below; celebratory EmptyState when all caught up. **Journey** (`journey-checklist.tsx`): progress-pulse header + a real **vertical stepper** (check nodes + connectors, overdue in red, hover edit/delete), friendly EmptyState with Start CTA when not started. All server-action wiring preserved. tsc clean; verified (Compliance shows Needs action 15 + Add/Missing rows; Journey shows Start-onboarding empty state).

**Phase 3 shipped (Details + Tasks):** added `DefGrid` + `GroupLabel` to the kit. **Details** tab regrouped into **Identity / Contact / Employment** definition grids (passport/national-ID still show their on-file document link) in one card with a single Edit→PersonForm; Documents + Manage as kit `SectionCard`s. **Tasks** tab: compact rows with a left **priority colour bar** (Critical/High/Medium/Low), title + code·company·status meta (closed struck-through), priority badge; recent activity as a tighter timeline; friendly EmptyState when none. Verified on a person with 17 tasks. tsc clean.

**Phase 4 shipped (pack folded into the drawer):** `PersonPackBuilder` (its own bottom-sheet Dialog) refactored into `PersonPackPanel` (embeddable, no Dialog). Person drawer footer "Prepare pack" now sets `packOpen`, which makes the drawer body a single **pack step** (hero stays; tab pill auto-hides; a Back arrow returns to the tabs); deep-link `?pack=1` opens it directly. Reasons reworked to real presets as a chip row — **Documents · Visa/Permit · Onboarding · Recruitment · Contract** — with the redundant **"Custom"** (the Customise-sections panel is the custom path) and **"Work reminder"** (now the one-tap footer Remind) reasons removed. Single-column kit layout, sticky Download PDF / Save draft footer. tsc clean; verified open/back in preview.

**Phase 5 shipped (task drawer on the kit):** `task-drawer.tsx` rebuilt from `ModalShell` onto `EntityDrawer` — status/urgency-tinted hero (code + company, title, status/priority/escalated badges), tabs **Overview · History**, Overview = meta grid (deadline editor, accountable, dept, category) + description + latest update + quick-update collapsible + source meeting, History = timeline. Action bar: Complete/Reopen (primary), Escalate (danger outline), and delete (with inline confirm strip) + Full-page as icon buttons. Person + Task drawers now share the exact same shell/kit. tsc clean; verified Overview/History in preview.

**Phase 6 shipped (tailored pack reasons):** the reason chips were showing identical content (the reason only pre-selected sections). Added `purposeRelevance(purpose)` in `person-pack-builder.tsx` — a per-reason predicate over a doc/requirement's text. `effPack` now filters the missing-docs gaps, expiry issues and on-file documents to what the reason is about (Visa/Permit → immigration/passport/permit; Recruitment → CV/certificates/ID/references/photo/transcript; Contract → contract/agreement/NDA; Documents + Onboarding → everything). The request checklist (`allGaps`) and the PDF `exclude` list respect the same filter so preview, draft message and PDF all match. Verified: Documents 13 items · Visa/Permit 2 · Recruitment 7 — genuinely different now. tsc clean.

**Drawer redesign complete (Phases 1–6).**

### Drawer kit rollout (2026-06, after the redesign)
- **Equipment held** card (`person-assets.tsx`) rebuilt tab-native on the kit (SectionCard header + rows with hover-reveal Return + `EmptyState`), no more standalone collapsible.
- **Company quick-view drawer** added: clicking a company card on `/companies` now opens an `EntityDrawer` pop-up (`company-drawer.tsx`, reads `?company=ID`, mounted in layout next to Person/Task drawers; opener `company-drawer-link.tsx`). Backed by new `GET /api/company-detail` (company fields + `buildCompanyRequirementScores` + task open/overdue + docs count/attention + team count, reusing `getAllTasks`/`listDocuments`). Hero (accent avatar, name, legal/reg, compliance + overdue badges), Overview tab (stat tiles → deep-link into the page tabs, compliance card, documents-needing-attention, open-tasks preview), action bar (Open full page · Files · Team). The full `/companies/[id]` page stays for editing; drawer is the glanceable quick view. tsc clean; verified on Terra Green.
- **Company page polish (done):** the shared `ComplianceSummaryCard` got a tone-coloured `ProgressTrack` + dot separators so the company Overview / Documents centre rhyme with the drawers' compliance card (owner chose "just polish rough spots", not a full rebuild).

### Parked for later (owner-confirmed, not started)
- **Full `/companies/[id]` page redesign** — a proper rebuild of the company page on the kit (beyond the rough-spot polish already done). Deferred to a future session.
- **Task module work** — the full task **page** (`/task/[code]`) and/or the Tasks **list/table** modernised on the kit (the task *drawer* is already done). Owner said "leave tasks, we'll work on it another time."
- **More company-drawer entry points (DONE):** `CompanyDrawerLink` now wired into the **task drawer** hero company name, **person drawer** company line, the **full task page** (`/task/[code]`) company link, and the **Insights** company rows — all open the quick-view drawer (`?company=ID`) instead of navigating; the link auto-closes any open task/person drawer. (Task table/board show company as plain text, no link.) Remaining optional spots: Org chart, Home, Director Brief. Verified: clicking "Dar Spices" in the DS-001 task drawer swaps `?task=` → `?company=1` and opens the company drawer.
