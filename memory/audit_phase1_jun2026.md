...
All seven phases implemented, type-checked (`tsc --noEmit` clean after every phase; full `next build` green with a 4 GB heap) and pushed. Commits: P0 `1338fe1`+`dc6785c` · P1 `81d13c6` · P2 `64dfcd1` · P3 `d46a22c` · P4 `a6d1dc3` · P5 `031e0a3` · P6 `a7ce035` · P7 `7546986`. Follow-up passes: 6.5-cache `012ef1c` · 3.2-buttons `2c1fc42`.
...
- **Phase 3 (design): done** — 3.1/3.5/3.6 (~16 dialogs → glass), 3.4 (card radii via shared primitives), 3.3 (shared drawer labels), **3.2 DONE** (49 standard buttons unified onto the shared `Button` across 36 files via a conservative fan-out, commit `2c1fc42`; 166 special cases — nav pills, segmented controls, round/icon buttons, the login pill, `.ics` anchors — deliberately skipped; tsc + full build green). **Deferred:** 3.7 extract one shared register-row.
...
- **Phase 6 (performance): safe wins done** — 6.1/6.2 (React `cache()` dedupes repeat documents/staff-ID/sites/roles reads per render), 6.3 (elkjs org engine lazy-loaded via `next/dynamic`), 6.4 (login logo via `next/image`), 6.8 (manager portal compliance scoped to direct reports). **6.5 safe slice DONE** (commit `012ef1c`: `cache()` on leave-type + holiday lookups, more per-render de-dup). **Deferred:** 6.5b aggressive page-caching / force-dynamic removal (staleness risk on a live ops tool — needs design), 6.6 list virtualisation (not needed at scale), 6.7 lazy-load other big client screens.
...
**Optional-items pass DONE (2026-06-13):** 4.14 (+ button on Assets/Calendar/Letters/Leave, commit 5a5be47), 4.18 (searchable manager pickers, 5a5be47), 2.5 (Agenda default on phones, 76692c5), 6.7 (GlobalDrawers lazy-load — defers person-drawer/pack/etc, 76692c5), 4.7 (conversation refetch-on-post, 6e99484), 1.16+4.13+2.3+4.20+7.5 (cosmetic batch, 6e5a76f), 5.1 (sick-leave cost on Brief, 7fb15fe) — plus 3.2 buttons + 6.5-cache earlier. All tsc + full build green; lazy drawers runtime-verified.

**Remaining backlog (deliberately deferred):** 7.1-file-splits + 3.7-table-row (high-risk internal refactors — do per-file, not in a batch) · 6.5b-revalidate (page-caching — stale-data risk on a live ops tool) · 7.3-ai-cancel (minor) · 6.6-virtualise (not needed at current scale) · 4.21-roster-visibility (owner decision). None are data-loss or security.
