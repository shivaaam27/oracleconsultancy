...
| Wispr-style voice — Phase 1 (engine + interface) | Done | Real audio recording transcribed by Groq Whisper `whisper-large-v3-turbo` via `/api/transcribe`, dictionary used as prompt bias. New `voice-button.tsx` with live level meter, timer, transcribing state, live captions (parallel browser recogniser, display-only), and browser-speech fallback. Used by Quick Capture, task updates, Meeting notes, and Ask COS. Phases 2-6 (intent/correction handling, punctuation/lists, self-learning dictionary, snippets, tone shaping) still pending. |
