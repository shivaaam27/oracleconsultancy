...
| 8. Reminders + To-do (in Workbook) | **Done.** New Workbook **To-do** tab (`WorkbookTodo`) alongside Meetings/Notes: every open task with a deadline, grouped Overdue/Today/Tomorrow/This week/Later. Tick to complete, snooze (SnoozeSheet), tap to open. Shares the tasks data (changes propagate). | **Done** |
