...
# Oracle System - V2 Status and Roadmap
...
| Ask Oracle meeting memory | Done | `/api/ask` includes relevant saved meetings/minutes/raw notes and linked task codes in context. |
...
| Voice intelligence layer | In progress | Shared dictation polishing action, Meeting/Quick Capture/task update voice flows, personal dictionary, and browser-language Ask Oracle dictation. |
...
| Page-aware assistant | Done | Floating Oracle assistant knows the current page via `src/lib/page-context.ts` and passes it to `/api/ask` (and `/api/action`). "This page / this task / here" now resolve; current task auto-focuses for pronoun commands. |
...
| Documents & Compliance | Done (Phases 1–5) | New `/documents` centre tracking licences/contracts/certificates/visas etc. with issue/expiry dates and per-doc reminder lead days. Migration `0017` (`documents` + `document_links`); derived status (Valid/Expiring/Expired/No expiry); helpers in `src/lib/documents.ts` + client-safe `documents-shared.ts`. Page = filterable table (company/category/status) with colour-coded expiry countdowns, peek + create/edit dialogs, and one-tap Renew→linked task. In the nav via the pill **More** sheet (`top-pill.tsx`). **Phase 3:** Oracle Home "Documents needing attention" card + expired/expiring counts in the daily push. **Phase 4:** in-app file uploads to a private Supabase Storage bucket (`documents`, 20 MB) via `storage_path`/`file_name` (migration `0018`) — choose/replace/remove in the form, paperclip on rows, signed-URL open; `next.config.ts` body limit raised to 25mb. **Phase 5 (complete):** AI auto-fill of document fields from pasted text (Groq + rule fallback, `extractDocumentFields`), Ask Oracle now answers compliance/expiry questions, "Compliance by company" section on `/insights`, and an Inbox "As document" button that opens the create dialog pre-extracted. Full plan: `documents_compliance_plan.md`. |
...
- **Voice should polish, not merely transcribe.** Dictation keeps rough capture fast, then cleans text with context and the Oracle vocabulary dictionary.
...
| Assistant polish (ChatGPT-style) | **Done.** Removed "Ask Oracle" from the sidebar (entry is the floating button + old /ask redirect). Mobile full-screen now **locks background scroll**; drag rebuilt with a controlled motion value (`y` + explicit snap) — up = full, down = minimise/close, velocity-aware. AskCOS redesigned: **input pill pinned at the bottom**, ChatGPT-style **empty home** (name greeting from new `settings.operatorName` + suggestion pills), **page-aware suggestions** (task/company context), a **＋ menu** (Attach this page · New task · New note), a **context chip** ("Asking about DS-005" with clear), **New chat**, verbose helper text removed, all on tokens (light/dark). Greeting name set in Settings → About you. | **Done** |
| Ask Oracle merged into the pill | **Done.** Ask Oracle now lives only in the floating assistant (`FloatingAssistant`). Three sizes sharing one mounted `AskCOS` (state preserved): desktop popover / mobile bottom-sheet (half) ⇄ **full-screen**, toggled by a Maximise/Minimise icon and animated via framer `layout`. Mobile: drag the grab handle — up → full, down → minimise/close (`useDragControls`, dragSnapToOrigin, velocity-aware). `/ask` page removed → client redirect to `/` that opens the assistant full-screen; sidebar "Ask Oracle" opens the pill instead of navigating. Full size uses the non-minimal AskCOS (quick-prompt chips, focused-task context chip, Clear/new-chat already present). | **Done** |
