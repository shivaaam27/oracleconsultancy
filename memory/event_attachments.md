...
# Papers that travel with an event — BUILT (Aug 2026, current Sept 2026)

> **Sept 2026 changes:** the public event page `/e/<token>` was **removed**
> (26 Sept 2026) — only the file streamer `src/app/e/[id]/doc/[docId]/route.ts`
> remains, still authorised by the event's share token. `publicEventUrl()` in
> `app/calendar/actions.ts` now returns `null`, so emails carry no "View this
> event" link; the per-event `.ics` is still served at `/api/calendar/<token>`.
> Documents live in **Files** (`/files`; `/documents` redirects), and an event
> paper lands in its company's folder automatically (see below).

Follows [[calendar]], which made every event reach Google and
...
| Where does an attached file live? | **In the document library (now Files)**, category "Attachment" — same convention task files follow |
...
have this" — which governs the email AND the permanent file link together, so
un-ticking it withdraws the file from both at once. Migration 0117 is applied.
...
| **Permanent link** | `/e/<token>/doc/<id>` streams the file (the `/e/<token>` page that listed them was removed Sept 2026). |
...
- **Reaching into the library is owner-only**: `linkEventDocumentAction`,
  ...
  document to an event and read it off the permanent link. The portal form shows no
  library picker (`allowLibrary` defaults false).
- **`documentIds` in the form is a second door to the same hole.**
  `attachableDocumentIds` in `app/portal/actions.ts` keeps only documents stamped
...
- **Administrator** → `/calendar` (Studio; `app/calendar/calendar-board.tsx`,
...
4. **Abandoned uploads littered the library.** Adding a file then removing it before
   saving left a document in the library for ever — the exact clutter the Aug 2026
...
- **Kept: "Join the meeting"** (a meeting email has exactly one button) and a quiet
  **"View this event/meeting ›"** text link. **Since the public page was removed
  (Sept 2026) that link is no longer emitted** (`publicUrl` is always null); the
  inline `text/calendar` part and `/api/calendar/<token>` are what remain for a
  guest whose calendar does not file invitations by itself.
...
## Verified
- 26 `event-read-core` tests + 8 ics-attach tests in the suite (`npm test`).
