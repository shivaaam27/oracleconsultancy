...
Agreed phased plan after the nav move: **1 (DONE)** HRMS nav tab + briefcase icon + unified "+". **2 (DONE)** Smart breadcrumbs — `src/components/hrms/hrms-crumbs.tsx` (`HrmsCrumbs from={searchParams.from}`): always "‹ HRMS", plus "‹ CODE" when `?from=task:CODE`. Added to `/people`, `/companies`, `/documents`, and `/companies/[id]` (replaced its hardcoded "Task Management" back-link). Task detail page links to the company with `?from=task:${code}`. **3** People deactivate — individual + **bulk** (decision: deactivate only, no hard delete; built on `people.active`). **4** Documents capture unified into one panel keeping all three inputs (Upload · Link · Paste text). **5** Documents smarter AI reading (scanned/photographed PDFs, images, handwriting, dirty docs → vision model) + **overflow-to-Notes** (unmapped extracted info auto-appended to Notes). Honour `getGroqKey()`.
