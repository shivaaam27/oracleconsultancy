...
- `chat-realtime.tsx` is now a hook `useThreadChannel(threadId, {onChange, meName})` returning `{typing, sendTyping}`; `onChange(type)` carries the broadcast event type.
- **Loop fix (critical)**: `markRead` must NEVER run inside the live-refetch path or it ping-pongs ("read" broadcast → peer refetch → peer markRead → broadcast → …). So: `threadStamp` is CONTENT-ONLY (no read pointer, else polling self-triggers); live refetch uses `refreshThread` (no markRead); the surface calls `markThreadRead` only on content events (`type !== "read"`); `openThread` (with markRead) is used only on initial select. Verified: socket connected, zero idle traffic, no storm.
- **Realtime needs a REDEPLOY to take effect in production**: `NEXT_PUBLIC_*` is baked into the client bundle at build time — setting it on Vercel does nothing until the next deploy. Locally, restart the dev server. Infra verified working (socket SUBSCRIBED, REST broadcast 202 + delivered).
