...
- Critical: `prepare: false` and `max: 1` are required because PgBouncer transaction mode doesn't support prepared statements. See [db/index.ts](../src/db/index.ts).
...
- Custom design tokens: `bg-bg`, `bg-bg-elev`, `bg-bg-muted`, `bg-bg-subtle`, `border-border`, `text-fg`, `text-fg-muted`, `text-fg-subtle`, `bg-accent`, `bg-danger`, `bg-warn`, `bg-success` â€” defined in `globals.css`.
...
- Env var: `GROQ_API_KEY`. Absent â†’ graceful fallback to rule-based polish or empty result.

## Spreadsheet ingest
- `xlsx` (SheetJS) loaded directly from sheetjs.com tarball (not the npm tombstone).

## Env vars
- `DATABASE_URL` â€” Supabase pooler URL (must be the 6543 pooled one).
- `GROQ_API_KEY` â€” optional but unlocks AI features.
- `XLSX_PATH` â€” optional override for import script, default `C:/Users/User/Downloads/Chief Of Staff Workflow - Live.xlsx`.
